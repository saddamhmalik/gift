        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-10">
          ...
          <div className="lg:col-span-2">
            ...
            <p className="text-sm text-slate-500 leading-relaxed mb-6 max-w-[280px]">
              India's smart payments &amp; rewards platform. Earn PayFlex Points on every purchase
              and redeem for real savings — <span className="text-slate-400">1 Point = ₹1.</span>
            </p>
            ...
            <ul className="space-y-3 text-xs">
              ...
              <li className="flex items-start gap-2.5">
                ...
                <span>
                  Unit No. 607, 6th Floor, Capital Business Park,
                  <br />
          ...
          {/* ── Shop ── */}
          <div>
            <h4 className="text-white text-sm font-bold mb-4 tracking-wide uppercase text-[11px] opacity-60">
              Shop
            </h4>
            <ul className="space-y-2.5 text-sm">
              {SHOP_LINKS.map(([label, to]) => (
                <li key={to}>
                  <Link
                    to={to}
                    className="hover:text-white hover:translate-x-0.5 inline-flex transition-all duration-150"
                  >
                    {label}
                  </Link>
                </li>
      ...
      <div className="border-t border-white/5 py-5">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs">
          <span className="text-slate-600">
            © {new Date().getFullYear()} SmartPayflex Payments Pvt. Ltd. All rights reserved.
          </span>
          <div className="flex items-center gap-4 text-slate-600">
            ...
            <Link to="/loyalty" className="hover:text-slate-300 transition-colors">
              Loyalty
            </Link>
            <Link to="/check-balance" className="hover:text-slate-300 transition-colors">
              Check Balance
            </Link>
          </div>
        </div>
