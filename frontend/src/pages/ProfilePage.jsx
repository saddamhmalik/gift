...
import {
  User, Mail, Phone, Lock, Camera, Trash2, CheckCircle, AlertCircle,
  Loader2, Eye, EyeOff, Edit2, Save, X, ShieldCheck, RefreshCw,
  BadgeCheck, Clock, KeyRound, LogOut,
} from 'lucide-react'
import { useAuth } from '../contexts/AuthContext'
import {
  updateProfile, uploadAvatar, removeAvatar,
  requestEmailChange, verifyEmailChange, resendEmailVerification,
  requestPhoneChange, verifyPhoneChange, changePassword,
} from '../api/profile'
...
export default function ProfilePage() {
  ...
  useEffect(() => {
    ...
    if (token && email && user) {
      verifyEmailChange(token, email)
        ...
        .catch(err => setEmailMsg({ type: 'error', text: err.response?.data?.message ?? 'Verification failed.' }))
    }
  }, []) // eslint-disable-line
  ...
  const handleRequestEmail = async () => {
    ...
    try {
      ...
      setEmailSent(true)
      setEmailMsg({ type: 'success', text: `Verification link sent to ${newEmail}. Check your inbox.` })
    } catch (e) {
      setEmailMsg({ type: 'error', text: e.response?.data?.message ?? 'Failed to send verification.' })
  ...
  const handleVerifyPhone = async () => {
    ...
    try {
      ...
      await refreshUser()
      setPhoneStep('edit'); setNewPhone(''); setPhoneOtp('')
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 sm:px-6 py-10">
      ...
      <div className="card p-6 mb-6">
        <div className="flex flex-col sm:flex-row items-center sm:items-start gap-6">
          ...
          <div className="flex-1 text-center sm:text-left">
            ...
            {user.member_since && (
              <p className="text-xs text-gray-400 mt-1">Member since {new Date(user.member_since).toLocaleDateString('en-IN', { month: 'long', year: 'numeric' })}</p>
            )}
        ...
        {avatarMsg && <div className="mt-4"><Alert type={avatarMsg.type}>{avatarMsg.text}</Alert></div>}
      </div>
      ...
      <Section title="Personal Information" icon={User}>
        ...
        {nameMsg && <div className="mt-3"><Alert type={nameMsg.type}>{nameMsg.text}</Alert></div>}
        <button
          onClick={handleSaveName}
          disabled={nameSaving || (!firstName.trim() && !lastName.trim())}
          className="mt-4 btn-primary !py-2.5 !rounded-xl disabled:opacity-50"
        >
          {nameSaving ? <><Loader2 size={14} className="animate-spin" /> Saving…</> : <><Save size={14} /> Save Name</>}
        </button>
      </Section>
      ...
      <Section title="Email Address" icon={Mail}>
        ...
        {!emailSent ? (
          <>
            <label className="block text-xs font-semibold text-gray-500 mb-1.5">Change to new email</label>
          ...
          <div className="flex items-center gap-2">
            <p className="text-sm text-gray-500 flex-1">Verification link sent to <span className="font-semibold text-gray-800">{newEmail}</span></p>
            <button onClick={() => { setEmailSent(false); setNewEmail('') }} className="text-xs text-gray-400 hover:text-gray-600 flex items-center gap-1">
              <RefreshCw size={11} /> Change
            </button>
          </div>
        )}

        {emailMsg && <div className="mt-3"><Alert type={emailMsg.type}>{emailMsg.text}</Alert></div>}
      </Section>
      ...
      <Section title="Phone Number" icon={Phone}>
        ...
        {phoneStep === 'edit' ? (
          <>
            <label className="block text-xs font-semibold text-gray-500 mb-1.5">Change to new phone</label>
            <div className="flex gap-2">
              <input
          ...
          <>
            <label className="block text-xs font-semibold text-gray-500 mb-1.5">Enter the 6-digit OTP sent to {newPhone}</label>
            <div className="flex gap-2">
              <input
              ...
              <button onClick={() => { setPhoneStep('edit'); setPhoneOtp(''); setDevOtp('') }}
                className="p-2 border-2 border-gray-200 rounded-xl hover:bg-gray-50 text-gray-500 transition-colors shrink-0">
                <X size={14} />
              </button>
            </div>
            <button onClick={handleRequestPhone} disabled={phoneLoading}
        ...
        {phoneMsg && <div className="mt-3"><Alert type={phoneMsg.type}>{phoneMsg.text}</Alert></div>}
      </Section>
        ...
        <div className="space-y-3">
          <div>
            ...
            <div className="relative">
              <input
                type={pwShow ? 'text' : 'password'} value={pwCurrent} onChange={e => setPwCurrent(e.target.value)}
              ...
              />
              <button type="button" onClick={() => setPwShow(v => !v)} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600" tabIndex={-1}>
          ...
          <div>
            ...
            <input
              type={pwShow ? 'text' : 'password'} value={pwNew} onChange={e => setPwNew(e.target.value)}
          ...
          <div>
            ...
            <input
              type={pwShow ? 'text' : 'password'} value={pwConfirm} onChange={e => setPwConfirm(e.target.value)}
      ...
      <div className="card p-6 border border-red-100">
        ...
        <p className="text-sm text-gray-500 mb-4">Sign out of your PayFlex account on this device.</p>
        <button
