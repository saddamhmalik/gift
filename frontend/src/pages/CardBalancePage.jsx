...
import {
  CreditCard, Search, CheckCircle, AlertCircle, Loader2,
  Calendar, Wallet, RefreshCw, Info, Eye, EyeOff,
} from 'lucide-react'
...
function StatCard({ icon: Icon, label, value, highlight = false }) {
  return (
    <div className={`rounded-2xl p-5 flex flex-col gap-1.5 border
      ${highlight
        ? 'bg-gradient-to-br from-primary-500 to-primary-700 border-primary-400 text-white'
        : 'bg-white border-gray-100 text-gray-800'}`}>
      <div className={`flex items-center gap-2 ${highlight ? 'text-primary-100' : 'text-gray-400'}`}>
        <Icon size={14} />
        <span className="text-xs font-medium">{label}</span>
      </div>
      <p className={`text-2xl font-extrabold leading-tight ${highlight ? 'text-white' : 'text-gray-900'}`}>
        {value}
      </p>
    </div>
  )
...
export default function CardBalancePage() {
  ...
  const handleCheck = async (e) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'Balance enquiry failed. Please check the card details and try again.')
    } finally {
      setLoading(false)
    }
  }
  ...
  const isActive   = cardStatus?.toUpperCase() === 'ACTIVATED' || cardStatus?.toUpperCase() === 'ACTIVE'
    ...
    <div className="min-h-[70vh] max-w-lg mx-auto px-4 sm:px-6 py-12">
      ...
      <div className="text-center mb-10">
        ...
        <p className="text-gray-500 mt-2 text-sm">
          Enter your gift card number to instantly check the remaining balance.
          No login required.
        </p>
      </div>
      ...
      {!result && (
        <form onSubmit={handleCheck} className="card p-6 space-y-4">
          ...
          <div>
            <label className="block text-sm font-semibold text-gray-700 mb-1.5">
              PIN <span className="text-gray-400 font-normal text-xs">(optional for most cards)</span>
            </label>
            <div className="relative">
              <input
          ...
          {result.currency && (
            <div className="card p-4 flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm font-semibold text-gray-700">{result.currency.code} ({result.currency.numericCode})</p>
              </div>
            </div>
          )}
          ...
          <div className="flex items-start gap-2">
            ...
            <div className="text-xs text-blue-700 space-y-1">
              ...
              <ul className="list-disc list-inside space-y-0.5 text-blue-600">
                ...
                <li>After a successful resend, the PIN may have been reset — use the new PIN from the resent message.</li>
              </ul>
            </div>
          </div>
        </div>
